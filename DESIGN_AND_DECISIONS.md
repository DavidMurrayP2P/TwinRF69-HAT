 ...
 * An alternate is to say that this link will always be too low bandwidth to be multi hop in any way, at least for IP data. In this case we could just simplify and say that all links were point to point and to negotiate an up and down band. So in this case, ona single node, one radio is always listening and the other is always in sending mode.

Initally, lets just try to support a udp telnet session or a ping over this link.